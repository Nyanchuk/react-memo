...
# ВТОРАЯ ДОМАШКА
# Предварительный тайминг работы - 15 часов;
# Фактически затраченное время - 14 часов;

1. Создание суперсил - 10 часов
    + Определить функциональность "суперсил" (подсказки и т.д.) - 30 мин
        1) Суперсила "ГЛАЗ БОГА"
            * Открывает все карты на 5 сек, приэтом таймер останавливается
        2) Суперсила "ВТОРАЯ ПОЛОВИНКА"
            * Подсвечивает обе парные карты на игровом поле, если игрок не выбрал первую карту
            * Подсвечивает вторую парную карту на игровом поле, если игрок выбрал первую карту
        3) Суперсила "УДАЧНЫЙ ХОД"
            * Если игрок сомневается в выборе следующей карты, он может активировать "суперсилу" (в случае неудачи, игроку будет предоставлена еще одна попытка, но суперсила "УДАЧНЫЙ ХОД" больше не будет доступка, т.к. игрок уже использовал этот шанс) - название такое, потому что после активации этой суперсилы, игрок не проиграет даже в случае несовпадения выбранных карт
    + Визуал "суперсил" - 1,5 часа
    + Отображение "СУПЕРСИЛ" только на 3 уровне сложности - 30 мин
    + Добавить описание каждой хэлпы при наведении на "суперсилу" - 1 час
    + Изменить вариант позиционирования описания
    + Добавить активное/неактивное состояние для суперсил (если суперсила недоступна, то у нее другой цвет)
    + Организовать логику работы "суперсил" - 5 часов
        + 1) Суперсила "ПРОЗРЕНИЕ"
        + 2) Суперсила "ВТОРАЯ ПОЛОВИНКА" / ДОПОЛНЕНА
        + 3) Суперсила "ПРАВО НА ОШИБКУ"
    + Внедрить новые АПИ-запросы на добавление лидеров на ЛИДЕРБОРД !!! 30 мин
2. Доработка лидерборда - 4 часа
    + Определить функциональность ачивок и добавить графу "ДОСТИЖЕНИЯ": 30 мин
        1) Игра пройдена без упрощенного режима (варианты актив/неатив)
        2) Игра пройдена без "Суперсил" (варианты актив/неатив)
    + Реализовать ачивки (в неативном состоянии) 30 мин
    + Добавить описание ачивок при наведении - 1 час
    + Сделать акцент на первой тройке победителей - 30 мин
    + Внедрить новые АПИ-запросы на получение лидеров на ЛИДЕРБОРД !!! 30 мин
    + Реализовать активацию ачивок для пользователя на основании пройденной игры: 1 час
        + 1) Игра пройдена без упрощенного режима (варианты актив/неатив)
        + 2) Игра пройдена без "Суперсил" (варианты актив/неатив)
...
+ Добавлена оценка времени работы в README-файл.
# Третья домашка
+ Суперсилы успешно реализованы и интегрированы в игру.
+ В лидерборде отображаются ачивки.
+ Ачивки присваиваются согласно заданным условиям (прохождение без суперсил или в сложном режиме).
+ Внешний вид страницы полностью соответствует макету.
+ Приложение работает без ошибок в консоли.
+ Решение полностью соответствует техническому заданию к реализации фичи.
+ Уточнено исходное техническое задание, оно соответствует новой версии приложения.
+ Добавлена оценка времени работы в README-файл.
